...
function displayResults(data) {
    const resultsContent = document.getElementById('results-content');
    const riskColor = data.riskScore > 75 ? 'red' : data.riskScore > 50 ? 'orange' : 'green';

    document.getElementById('risk-score').innerHTML = `<span style="color: ${riskColor}">${data.riskScore}/100</span>`;
    document.getElementById('risk-description').value = data.riskDescription || "Moderate risk due to broad permissions.";
...
function getReportData() {
    const permissions = Array.from(document.querySelectorAll('.permission-note')).map(note => ({
    ...
    return {
        report: {
            riskScore: {
                value: parseInt(document.getElementById('risk-score').textContent.split('/')[0]),
